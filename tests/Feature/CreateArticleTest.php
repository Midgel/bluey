...
class CreateArticleTest extends TestCase
{
    ...
    // Simula o S3 bucket para não fazer uploads reais durante os testes
    ...
    {
        // Preparação  - Preparar os dados necessários para teste de criação de artigo
        ...
        // Ação - Executar a ação com o token correto
        $response = $this->post('/admin/articles', $articleData);

        // Assertividade - Verificar os resultados
        ...
        // 4. Verifica se a imagem foi enviada para o S3 (fake)
    ...
    {
        // Ação - Tentar criar artigo sem dados obrigatórios (mas com token correto)
        $response = $this->post('/admin/articles', [
            'token' => 'seu-token-secreto'
        ]);

        // Assertividade - Verificar se há redirecionamento (Laravel redireciona em caso de erro de validação)
    ...
    {
        // Preparação - Criar um artigo existente com todos os campos obrigatórios
        ...
        // Ação - Tentar criar outro artigo com o mesmo título
        $response = $this->post('/admin/articles', [
            'title' => 'Título Já Existente',
            'body' => 'Novo conteúdo',
            'author' => 'Novo Autor',
        ...
        // Assertividade - Verificar se houve erro de validação (status 302 = redirecionamento com erro)
        $response->assertStatus(302);

        // Deve existir apenas o primeiro post criado
        $this->assertEquals(1, Post::count());
    }
